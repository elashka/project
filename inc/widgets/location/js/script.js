var sowl = window.sowl || {};

sowl.LocationGoogleMap = function($) {
    return {
        ...
        initMaps: function() {
            ...
            var $autoCompleteFields = $( '.sow-location-autocomplete' );
            ...
            if( $autoCompleteFields.length === 0 ) {
                ...
                $autoCompleteFields.each(function (index, element) {
                    ...
                    var $mapField = $(element).siblings('.sow-location-map-canvas');
                    ...
                    if ($mapField.length > 0) {
                        ...
                        this.getLocation(options.address).done(
                            ...
                            $mapField.append('<div><p><strong>' + soWidgetsLocationGoogleMap.geocode.noResults + '</strong></p></div>');
            ...
            autoCompleteInit.always(function(){
                $('.sow-location-map-canvas').each(function (index, element) {
                    var $$ = $(element);
                    ...
                    this.getLocation( address ).done(
                        ...
                        $$.append( '<div><p><strong>' + soWidgetsLocationGoogleMap.geocode.noResults + '</strong></p></div>' );
                    } );
...
function soLocationGoogleMapInitialize() {
    new sowl.LocationGoogleMap(jQuery).initMaps();
}

jQuery(function ($) {

    sowl.setupGoogleMaps = function() {
        var libraries = [];
        var apiKey;
        $('.sow-location-map-canvas').each(function(index, element) {
            var $this = $(element);
        ...
        if ( mapsApiLoaded ) {
            soLocationGoogleMapInitialize();
        } else {
            var apiUrl = 'https://maps.googleapis.com/maps/api/js?callback=soLocationGoogleMapInitialize';
            ...
            if ( window.console && window.console.error ) {
                ...
                sowl.onLoadMapsApiError = function ( error ) {
                ...
                window.console.error = sowl.onLoadMapsApiError;
            }

            $( 'body' ).append( '<script async type="text/javascript" src="' + apiUrl + '">' );
        }
    };
    sowl.setupGoogleMaps();
    $( sowl ).on( 'setup_widgets', sowl.setupGoogleMaps );
});
